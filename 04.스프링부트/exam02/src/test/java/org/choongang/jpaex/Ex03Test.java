...
//        repository.save(data);  //persist(data) 같은 결과
//        repository.flush();
        data = repository.saveAndFlush(data);  // persist(data) + flush() 같이 호출 - 변화 감지
        // 반환값 - 영속 상태

        data.setSubject("(수정)제목");
        //repository.saveAndFlush(data);
        //repository.flush();
        repository.save(data); // 영속상태

        // 조회실행시 암묵적으로 flush()가 먼저 수행 -> 조회
